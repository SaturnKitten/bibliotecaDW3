...
const insertFuncionario = async (req, res) => {
    try {
        ...
        res.json({status: "ok", mensagem: "Funcionário inserido com sucesso", registro: novoFuncionario});
    }
    catch (error) {
        ...
        if (error.code === '23505') {
            return res.status(409).json({
                status: "error", 
                mensagem: "Username ou Email já cadastrado no sistema." 
            });
        }
        // Erro genérico
...
const updateFuncionario = async (req, res) => {
    try {
        ...
        if(Object.keys(dadosFuncionario).length === 0){
            return res.status(400).json({ status: "error", mensagem: "Nenhum dado fornecido para atualização." });
        }

        const funcionarioAtualizado = await mdlFuncionario.updateFuncionario(idFuncionario, dadosFuncionario);
